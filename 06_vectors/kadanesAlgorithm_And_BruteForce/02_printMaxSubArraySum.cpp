/*
    ...
    This approach calculates the maximum sum of any contiguous subarray within a
   given array. It uses nested loops to consider all possible subarrays:
    ...
    For each subarray, the sum is computed and compared to the current maximum
   sum. The overall time complexity is O(n^3), where n is the size of the array,
   due to the three nested loops.
...
/**
 * The maxSubArray2 function computes the maximum sum of a contiguous subarray
 * within a given integer array using a brute-force approach with a time
 * complexity of O(n²). It iterates through all possible subarrays, calculates
 * their sums, and keeps track of the maximum sum encountered.
 */
...
int main() {
  ...
  cout << "brute force approach big of n^2: " << maxSubArray2(arr, size)
       << endl;
